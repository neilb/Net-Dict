...
In addition to looking up word definitions,
...
would find all words which start with "anti" in all databases:
...
The debug level - a non-zero value will result in debugging
...
Looks for words which match C<$pattern> according to the specified
matching C<$strategy>.
...
The user should be known to the server, and C<$PASSPHRASE>
...
specified capability; false (zero) otherwise. E.g.:
